...
class WarblerProjectIntelligence:
    def __init__(self, gemma_endpoint="http://localhost:11434"):
    ...
    def analyze_project_request(self, user_request):
        """
        Use Gemma3 to intelligently analyze project setup requests
...
"""
                ...
                if response.status_code == 200:
                    ...
                    if message_content:
                        try:
                            ...
                            return self._enhance_analysis(analysis, user_request)
                            ...
                            safe_print("⚠️ AI response couldn't be parsed as JSON, using enhanced fallback")
                            return self._get_enhanced_fallback_analysis(user_request)
                    else:
        ...
        safe_print(f"[AI] No endpoints available, using enhanced fallback analysis")
        return self._fallback_analysis(user_request)
    
    def _enhance_analysis(self, gemma_analysis, user_request):
        """Enhance Gemma3 analysis with Warbler-specific insights"""
        
        # Add Warbler's expertise
        enhanced = gemma_analysis.copy()
        enhanced['warbler_enhancement'] = {
            'tlda_integration': True,
            'auto_documentation': True,
            'performance_monitoring': True,
            'suggested_tldl_tags': self._suggest_tldl_tags(gemma_analysis.get('game_type', 'custom')),
            'development_milestones': self._create_milestones(gemma_analysis),
            'testing_strategy': self._suggest_testing_approach(gemma_analysis)
        }
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python warbler_project_intelligence.py 'project request' [--generate-files]")
        sys.exit(1)
