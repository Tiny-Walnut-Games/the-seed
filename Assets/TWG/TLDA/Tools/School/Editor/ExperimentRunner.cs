...
namespace TWG.TLDA.School.Editor
{
    ...
    public class ExperimentRunner : EditorWindow
    {
        ...
        private StringBuilder executionLogs = new StringBuilder();
        ...
        private void DrawResultsSection()
        {
            ...
            if (lastExecutionTime != default(DateTime))
            {
        ...
        private ExperimentManifest LoadManifestFromFile(string filePath)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    manifest.Name = trimmed.Substring(5).Trim().Trim('"');
                }
                else if (trimmed.StartsWith("hypothesis_type:"))
                {
                    manifest.HypothesisType = trimmed.Substring(16).Trim();
                }
                else if (trimmed.StartsWith("hypothesis_id:"))
                {
                    manifest.HypothesisId = trimmed.Substring(14).Trim();
                }
        ...
        private async void ExecuteAllExperiments()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"Experiment execution failed: {ex}");
            }
            finally
            {
                isExecuting = false;
        ...
        private async Task<ProcessResult> ExecutePythonHarness(string manifestPath, string outputDir)
        {
        ...
        private RunMetadata CreateRunMetadata(ExperimentManifest manifest, string runId, ProcessResult result)
        {
        ...
        private async Task GenerateGitHubIntegrationData()
        {
            var integrationData = new GitHubIntegrationData
            {
