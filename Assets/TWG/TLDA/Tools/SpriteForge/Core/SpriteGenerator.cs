...
namespace TWG.TLDA.SpriteForge
{
    ...
    public static class SpriteGenerator
    {
        ...
        /// KeeperNote: Main entry point for deterministic sprite generation. Now integrates with modular system.
        /// </summary>
        public static SpriteGenerationResult GenerateSprite(string seed, SpriteSpec spec)
        {
            try
            {
                // Check if modular generation is requested or beneficial
                if (ShouldUseModularGeneration(spec))
                {
                    Debug.Log($"[SpriteGenerator] Using modular generation for {spec.Archetype} {spec.Genre}");
                    return ModularSpriteGenerator.GenerateCompatibleSprite(seed, spec);
                }
                
                // Fallback to legacy generation for compatibility
                Debug.Log($"[SpriteGenerator] Using legacy generation for {spec.Archetype} {spec.Genre}");
                return GenerateLegacySprite(seed, spec);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Generate sprite using legacy algorithm for backwards compatibility
        /// </summary>
        public static SpriteGenerationResult GenerateLegacySprite(string seed, SpriteSpec spec)
        {
            try
            {
                ...
                var result = new SpriteGenerationResult
                {
                    ...
                    SpriteSheet = GenerateSpriteSheet(spec, random),

                    // Generate animation data
                    AnimationData = GenerateAnimationData(spec, random)
                };
            ...
            {
                Debug.LogError($"[SpriteGenerator] Failed to generate legacy sprite: {ex.Message}");
                return null;
            }
        }
        
        /// <summary>
        /// Determine if modular generation should be used
        /// </summary>
        private static bool ShouldUseModularGeneration(SpriteSpec spec)
        {
            // Use modular generation for:
            // 1. Faculty ultra-rares (always get the best treatment)
            // 2. High rarity creatures (Epic, Legendary)
            // 3. Evolution chains (benefit from part-based composition)
            // 4. Creatures with shader effects enabled
            
            return spec.Faculty != FacultyRole.None 
                   || spec.Rarity >= RarityTier.Epic 
                   || spec.GenerateEvolutionChain 
                   || spec.HasShaderEffects;
        }
        ...
        private static Texture2D GenerateStandardSpriteSheet(SpriteSpec spec, System.Random random)
        {
            ...
            {
                filterMode = FilterMode.Point // Pixel-perfect
            };
