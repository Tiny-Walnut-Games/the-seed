...
namespace TWG.TLDA.SpriteForge.Editor
{
    ...
    public class SpriteForgeEditor : EditorWindow
    {
        ...
        private SpriteSpec currentSpec = new();
        private SpriteGenerationResult lastResult;
        private ModularSpriteResult lastModularResult;
        ...
        private bool useModularGeneration = true;
        private bool showPartLibrary = false;
        private bool showPalettePreview = false;
        private bool showRiggedExport = false;
        private RiggedExportFormat riggedFormat = RiggedExportFormat.Spine;
        private PartType selectedPartType = PartType.Body;
        private List<PartTemplate> availableParts = new List<PartTemplate>();
        ...
        private void RefreshAvailableParts()
        {
            try
            {
                availableParts = ModularSpriteGenerator.GetAvailableParts(currentSpec.Archetype, currentSpec.Genre);
            }
            catch (System.Exception)
            {
                // Modular system might not be fully initialized
                availableParts = new List<PartTemplate>();
            }
        }
        ...
        private void OnGUI()
        {
            ...
            if (showPartLibrary)
            {
                DrawPartLibrarySection();
            }
            
            if (showPalettePreview)
            {
                DrawPalettePreviewSection();
            }
            
            if (showRiggedExport)
            {
                DrawRiggedExportSection();
            }
        ...
        private void GenerateSprite()
        {
            try
            {
                ...
                if (useModularGeneration)
                {
                    // Generate modular sprite
                    lastModularResult = ModularSpriteGenerator.GenerateModularSprite(testSeed, currentSpec);
                    
                    if (lastModularResult != null && lastModularResult.CompositeTexture != null)
                    {
                        // Convert to compatible format for display
                        lastResult = ModularSpriteGenerator.GenerateCompatibleSprite(testSeed, currentSpec);
                        Debug.Log($"[SpriteForgeEditor] Generated modular sprite with {lastModularResult.Parts.Count} parts");
                    }
                    else
                    {
                        Debug.LogError("[SpriteForgeEditor] Modular sprite generation failed");
                    }
                }
                else
                {
                    // Use legacy generation
                    lastResult = SpriteGenerator.GenerateLegacySprite(testSeed, currentSpec);
                    lastModularResult = null; // Clear modular result
                    
                    if (lastResult != null && lastResult.IsValid)
                    {
                        Debug.Log($"[SpriteForgeEditor] Generated legacy sprite: {lastResult.TokenMetadata.name}");
                    }
                    else
                    {
                        Debug.LogError("[SpriteForgeEditor] Legacy sprite generation failed");
                    }
                }
            }
            catch (Exception ex)
        ...
        private string GenerateTLDASeed()
        {
            ...
            return ByteArrayToHexString(hash)[ ..16 ].ToLower();
        }
        
        // Helper method for .NET Framework 4.7.1 compatibility
        ...
        // ===== MODULAR SYSTEM UI METHODS =====
        
        private void DrawModularOptionsSection()
        {
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("🧩 Modular Generation Options", EditorStyles.boldLabel);
            
            useModularGeneration = EditorGUILayout.Toggle("Use Modular Generation", useModularGeneration);
            
            if (useModularGeneration)
            {
                EditorGUILayout.HelpBox("Modular generation uses the new part-based composition system with advanced palette management and rigged animation support.", MessageType.Info);
                
                EditorGUILayout.BeginHorizontal();
                showPartLibrary = EditorGUILayout.Toggle("Show Part Library", showPartLibrary);
                showPalettePreview = EditorGUILayout.Toggle("Show Palette Preview", showPalettePreview);
                EditorGUILayout.EndHorizontal();
                
                EditorGUILayout.BeginHorizontal();
                showRiggedExport = EditorGUILayout.Toggle("Show Rigged Export", showRiggedExport);
                riggedFormat = (RiggedExportFormat)EditorGUILayout.EnumPopup("Export Format", riggedFormat);
                EditorGUILayout.EndHorizontal();
            }
            else
            {
                EditorGUILayout.HelpBox("Legacy generation uses the original algorithm for backwards compatibility testing.", MessageType.Warning);
            }
            
            EditorGUILayout.Space();
        }
        
        private void DrawPartLibrarySection()
        {
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("📚 Part Library", EditorStyles.boldLabel);
            
            EditorGUILayout.BeginHorizontal();
            selectedPartType = (PartType)EditorGUILayout.EnumPopup("Part Type", selectedPartType);
            ...
            EditorGUILayout.LabelField($"Available parts for {currentSpec.Archetype} ({currentSpec.Genre}):");
            
            var filteredParts = availableParts.FindAll(p => p.Type == selectedPartType);
            ...
            if (filteredParts.Count > 0)
            {
                ...
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField($"• {part.PartName}", GUILayout.Width(150));
                    EditorGUILayout.LabelField($"Layer: {part.Layer}", GUILayout.Width(100));
                    EditorGUILayout.LabelField($"Size: {part.Size.x}×{part.Size.y}", GUILayout.Width(80));
            ...
            {
                EditorGUILayout.HelpBox($"No {selectedPartType} parts available for this archetype/genre combination.", MessageType.Info);
            }
            
            EditorGUILayout.Space();
        }
        
        private void DrawPalettePreviewSection()
        {
            EditorGUILayout.Space();
            ...
            try
            {
                var palette = PaletteManager.Instance.GetPartPalette(selectedPartType, currentSpec.Genre, EvolutionStage.Adult);
                
                EditorGUILayout.LabelField($"3-Color Palette for {selectedPartType} ({currentSpec.Genre}):");
                
                EditorGUILayout.BeginHorizontal();
                var rect1 = GUILayoutUtility.GetRect(60, 30);
                var rect2 = GUILayoutUtility.GetRect(60, 30);
                var rect3 = GUILayoutUtility.GetRect(60, 30);
                
                EditorGUI.DrawRect(rect1, palette[0]);
                EditorGUI.DrawRect(rect2, palette[1]);
                EditorGUI.DrawRect(rect3, palette[2]);
                EditorGUILayout.EndHorizontal();
                
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Primary", GUILayout.Width(60));
                EditorGUILayout.LabelField("Secondary", GUILayout.Width(60));
                EditorGUILayout.LabelField("Accent", GUILayout.Width(60));
                EditorGUILayout.EndHorizontal();
                
                // Faculty palette preview
                if (currentSpec.Faculty != FacultyRole.None)
                {
                    EditorGUILayout.Space();
                    var facultyPalette = PaletteManager.Instance.GetFacultyPalette(currentSpec.Faculty, selectedPartType);
                    
                    EditorGUILayout.LabelField($"Faculty Palette ({currentSpec.Faculty}):");
                    
                    EditorGUILayout.BeginHorizontal();
                    var frect1 = GUILayoutUtility.GetRect(60, 30);
                    var frect2 = GUILayoutUtility.GetRect(60, 30);
                    var frect3 = GUILayoutUtility.GetRect(60, 30);
                    
                    EditorGUI.DrawRect(frect1, facultyPalette[0]);
                    EditorGUI.DrawRect(frect2, facultyPalette[1]);
                    EditorGUI.DrawRect(frect3, facultyPalette[2]);
                    EditorGUILayout.EndHorizontal();
                }
            }
            catch (System.Exception ex)
            {
                EditorGUILayout.HelpBox($"Palette preview error: {ex.Message}", MessageType.Warning);
            }
            
            EditorGUILayout.Space();
        }
        
        private void DrawRiggedExportSection()
        {
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("🦴 Rigged Animation Export", EditorStyles.boldLabel);
            
            if (lastModularResult != null)
            {
                EditorGUILayout.LabelField($"Export Format: {riggedFormat}");
                
                if (GUILayout.Button($"Export as {riggedFormat}"))
                {
                    ExportRiggedAnimation();
                }
                
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("Bone Structure Preview:");
                
                if (lastModularResult.BoneStructure != null)
                {
                    foreach (var bone in lastModularResult.BoneStructure.Bones)
                    {
                        EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField($"• {bone.Name}", GUILayout.Width(100));
                        EditorGUILayout.LabelField($"Parent: {bone.Parent ?? "root"}", GUILayout.Width(100));
                        EditorGUILayout.LabelField($"Pos: ({bone.Position.x:F1}, {bone.Position.y:F1})", GUILayout.Width(120));
                        EditorGUILayout.EndHorizontal();
                    }
                }
            }
            ...
            {
                EditorGUILayout.HelpBox("Generate a modular sprite first to enable rigged export.", MessageType.Info);
            }
            
            EditorGUILayout.Space();
        }
        
        private void ExportRiggedAnimation()
        {
            try
            {
                var exportData = ModularSpriteGenerator.GenerateRiggedExport(lastModularResult, riggedFormat);
                
                var outputPath = EditorUtility.SaveFilePanel(
                    $"Export {riggedFormat} Animation", 
                    Application.dataPath, 
                    $"{currentSpec.TokenId}_{riggedFormat}", 
                    riggedFormat == RiggedExportFormat.Spine ? "json" : "dbproj");
                ...
                if (!string.IsNullOrEmpty(outputPath))
                {
                    ...
                    Debug.Log($"[SpriteForgeEditor] Exported {riggedFormat} animation to: {outputPath}");
                    EditorUtility.DisplayDialog("Export Complete", 
                        $"Successfully exported {riggedFormat} animation data to:\n{outputPath}", 
                        "OK");
                }
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[SpriteForgeEditor] Rigged export failed: {ex.Message}");
                EditorUtility.DisplayDialog("Export Error", $"Failed to export rigged animation:\n{ex.Message}", "OK");
            }
        }
    }
}
