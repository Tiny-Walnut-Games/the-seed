    ...
    public class AlchemistManifestGenerator : EditorWindow
    {
        ...
        private void FetchIssueData()
        {
            ...
            try
            {
                ...
                // we'll generate mock data for demonstration
                currentIssueData = GenerateMockIssueData();
        ...
        private GilHubIssueData GenerateMockIssueData()
        {
            // Generate mock data for testing since we can't make HTTP calls in Unity Editor
            return new GilHubIssueData
            {
                IssueNumber = int.Parse(issueNumber),
                Title = $"Combat system narrative integration - Issue #{issueNumber}",
                Logline = "Combat system feels disconnected from narrative progression",
                Tension = "Players invest in story but battles feel like separate minigame",
                IrreversibleShift = "Integrate XP gain with narrative beats - combat outcomes affect story branches",
                MeasurableResidue = @"- Player engagement time in combat +15%
...
- Tutorial skip rate decrease",
                ...
                Labels = new[] { "gu-pot:distilled", "narrative", "alchemist:ready" }
            };
        }
