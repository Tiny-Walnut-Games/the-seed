...
public class MindCastleSceneSetup : MonoBehaviour
{
    ...
    private List<STAT7Node> allNodes = new List<STAT7Node>();
    private List<LineRenderer> adjacencyLines = new List<LineRenderer>();

    // Entity templates for sample generation
    private EntityTemplate[] entityTemplates;
    ...
    {
        entityTemplates = new EntityTemplate[]
        {
    ...
    public void SetupCompleteScene()
    {
        ...
        Debug.Log($"✅ MindCastle Scene Setup Complete! Created {allNodes.Count} entities with {adjacencyLines.Count} relationships");
    }
    ...
    {
        lineRendererParent = new GameObject("MindCastle_AdjacencyLines");
    ...
    private void CreateSampleEntities()
    {
        ...
        var lucaTemplate = entityTemplates[0];
        var lucaNode = CreateEntityNode(lucaTemplate);
        SetupLUCAEntity(lucaNode);
        allNodes.Add(lucaNode);

        // Create other entities
        for (int i = 1; i < Mathf.Min(numberOfSampleEntities, entityTemplates.Length); i++)
        {
            var template = entityTemplates[i];
        ...
        for (int i = entityTemplates.Length; i < numberOfSampleEntities; i++)
        {
    ...
    private void CreateEntityRelationships()
    {
        ...
        if (allNodes.Count > 0)
        {
            var lucaNode = allNodes[0];
            for (int i = 1; i < Mathf.Min(5, allNodes.Count); i++)
            {
                CreateAdjacency(lucaNode, allNodes[i]);
            }
        }
        ...
        for (int i = 1; i < allNodes.Count; i++)
        {
            ...
            int connectionCount = Random.Range(1, Mathf.Min(4, allNodes.Count - 1));
            for (int j = 0; j < connectionCount; j++)
            {
                int targetIndex = Random.Range(1, allNodes.Count);
                if (targetIndex != i)
                {
                    CreateAdjacency(currentNode, allNodes[targetIndex]);
                }
            }
        }

        Debug.Log($"🔗 Created {adjacencyLines.Count} adjacency relationships");
    }
    ...
    private void CreateAdjacencyLine(Vector3 start, Vector3 end)
    {
        ...
        var lineObj = new GameObject($"AdjacencyLine_{adjacencyLines.Count}");
        lineObj.transform.SetParent(lineRendererParent.transform);
        ...
        adjacencyLines.Add(lineRenderer);
    }
    ...
    private void CreateControlPanel(GameObject canvas)
    {
        ...
        // Add info text
        var infoText = CreateUIText(panelObj.transform, $"Entities: {allNodes.Count}\nRelationships: {adjacencyLines.Count}\n\nClick nodes to select\nScroll to zoom selected",
    ...
    private void FinalizeScene()
    {
        ...
        Debug.Log($"   📊 Entities: {allNodes.Count}");
        Debug.Log($"   🔗 Relationships: {adjacencyLines.Count}");
    ...
    {
        for (int i = 0; i < adjacencyLines.Count; i++)
        {
            var line = adjacencyLines[i];
            if (line != null)
            {
    ...
    /// </summary>
    public List<STAT7Node> GetAllNodes() => allNodes;

    /// <summary>
    /// Gets a specific node by entity ID
    /// </summary>
    public STAT7Node GetNodeById(string entityId) => allNodes.FirstOrDefault(n => n.EntityId == entityId);
