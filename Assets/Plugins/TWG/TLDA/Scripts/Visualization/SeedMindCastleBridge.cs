...
namespace TWG.Seed.Integration
{
    ...
    public class SeedMindCastleBridge : MonoBehaviour
    {
        ...
        private ISeedEngine _seedEngine;
        private Dictionary<string, NarrativeEntity> _spawnedEntities = new Dictionary<string, NarrativeEntity>();
        private bool _isVisualizationActive = true;
        ...
        async void InitializeSeedEngine()
        {
            ...
            _seedEngine = await SeedEngineFactory.CreateAsync();
        ...
        async Task PopulateInitialEntities()
        {
            ...
            var nearbyEntities = await _seedEngine.GetEntitiesInProximity(playerPos, 50f);
        ...
        {
            var playerPos = Camera.main?.transform.position ?? Vector3.zero;
            var nearbyEntities = await _seedEngine.GetEntitiesInProximity(playerPos, 100f);

            // Spawn new entities
            foreach (var entity in nearbyEntities)
            {
                if (!_spawnedEntities.ContainsKey(entity.Id))
                {
                    SpawnNarrativeEntity(entity);
                }
            }
        ...
        void SpawnNarrativeEntity(SeedEntity entity)
        {
            ...
            _spawnedEntities[entity.Id] = narrativeEntity;
        ...
        {
            var results = await _seedEngine.SearchEntities(query);
        ...
        {
            if (_spawnedEntities.TryGetValue(entityId, out var entity))
            {
        ...
        public async Task RegisterNewEntity(string content, string realm = "narrative")
        {
            ...
            var success = await _seedEngine.RegisterEntity(newEntity);
            if (success)
            {
    ...
    {
        private List<SeedMindCastleBridge.SeedEntity> _mockEntities = new List<SeedMindCastleBridge.SeedEntity>();
        ...
        void GenerateMockEntities()
        {
            ...
            for (var i = 0; i < 50; i++)
            {
                _mockEntities.Add(new SeedMindCastleBridge.SeedEntity
                {
                    Id = $"mock_{i}",
        ...
        {
            var entity = _mockEntities.FirstOrDefault(e => e.Stat7Address == stat7Address);
        ...
        public Task<IEnumerable<SeedMindCastleBridge.SeedEntity>> GetEntitiesInProximity(Vector3 position, float radius)
        {
            ...
            var count = Mathf.Min(10, _mockEntities.Count);
            var results = _mockEntities.OrderBy(_ => Random.value).Take(count);
            return Task.FromResult(results);
        }
    }
}
