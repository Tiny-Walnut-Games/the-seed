    ...
    public class UnityTestSuiteManager : EditorWindow
    {
        ...
        private async Task RunSymbolicLinterAsync(TestSuiteResults results)
        {
            ...
            else
            {
                ...
                {
                    LogMessage($"📄 Symbolic Linter output:\n{result.Output}");
                }
            }
        ...
        private async Task RunTLDLValidationAsync(TestSuiteResults results)
        {
            ...
            if (result.Success)
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 TLDL validation results:\n{result.Output}");
                }
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 TLDL validation output:\n{result.Output}");
                }
            }
            ...
            if (result.Output.Contains("WARNING"))
            {
                var warningCount = result.Output.Split("WARNING", StringSplitOptions.None).Length - 1;
                results.Warnings += warningCount;
                LogMessage($"⚠️ TLDL validation warnings: {warningCount}");
            }
        }
        ...
        private async Task RunDebugOverlayValidationAsync(TestSuiteResults results)
        {
            ...
            if (result.Success)
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 Debug overlay validation results:\n{result.Output}");
                }
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 Debug overlay validation output:\n{result.Output}");
                }
            }
            ...
            if (result.Output.Contains("WARNING"))
            {
                var warningCount = result.Output.Split("WARNING", StringSplitOptions.None).Length - 1;
                results.Warnings += warningCount;
                LogMessage($"⚠️ Debug overlay validation warnings: {warningCount}");
            }
        }
        ...
        private async Task RunSystemLinterAsync(TestSuiteResults results)
        {
            ...
            if (result.Success)
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 System linter analysis:\n{result.Output}");
                }
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(result.Output))
                {
                    LogMessage($"📄 System linter output:\n{result.Output}");
                }
            }
            ...
            if (result.Output.Contains("WARNING"))
            {
                var warningCount = result.Output.Split("WARNING", StringSplitOptions.None).Length - 1;
                results.Warnings += warningCount;
                LogMessage($"⚠️ System linter warnings: {warningCount}");
            }
        }
        ...
        private void ExportTestReport()
        {
            ...
            // Save to both the standard location and allow custom export
            var standardPath = "Assets/TLDA-Unity-Test-Results/test-report.txt";
            var standardDir = Path.GetDirectoryName(standardPath);
            ...
            try
            {
                // Always save to standard location
                File.WriteAllText(standardPath, report);
                LogMessage($"📄 Test report saved to: {standardPath}");
                
                // Also offer custom export
                var customPath = EditorUtility.SaveFilePanel("Export Test Report", "", "test-report.txt", "txt");
                
                if (!string.IsNullOrEmpty(customPath))
                {
                    File.WriteAllText(customPath, report);
                    LogMessage($"📄 Test report also exported to: {customPath}");
                }
                
                EditorUtility.DisplayDialog("Export Complete", 
                    $"Test report saved to:\n{standardPath}\n\n" +
                    (string.IsNullOrEmpty(customPath) ? "" : $"Also exported to:\n{customPath}"), 
                    "OK");
            }
            catch (Exception ex)
            {
                LogMessage($"❌ Failed to export test report: {ex.Message}");
                EditorUtility.DisplayDialog("Export Failed", $"Failed to export test report:\n{ex.Message}", "OK");
            }
        }
        ...
        private string GenerateTestReport()
        {
            ...
            report.AppendLine($"Project Path: {Application.dataPath}");
            report.AppendLine();
            report.AppendLine("=== Test Categories Executed ===");
            if (runSymbolicLinter) report.AppendLine("✓ Symbolic Linter");
            if (runDocumentationValidation) report.AppendLine("✓ TLDL Documentation Validation");
            if (runDebugOverlayValidation) report.AppendLine("✓ Debug Overlay Validation");
            if (runSystemLinter) report.AppendLine("✓ System Architecture Linter");
            if (runUnitySpecificTests) report.AppendLine("✓ Unity-Specific Tests");
            report.AppendLine();
        ...
        private void LogMessage(string message)
        {
            ...
            testLogs.AppendLine(logEntry);
            
            // Use proper Debug.Log formatting to avoid (object) display
