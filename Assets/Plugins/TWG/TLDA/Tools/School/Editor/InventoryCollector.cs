...
namespace TWG.TLDA.School.Editor
{
    ...
    public class InventoryCollector : EditorWindow
    {
        ...
        /// <summary>
        /// Run the inventory scan without opening any windows. Returns output path and surface count.
        /// </summary>
        public static async Task<(string outputPath, int surfaceCount)> RunHeadless()
        {
            var temp = ScriptableObject.CreateInstance<InventoryCollector>();
            try
            {
                ...
                await temp.ScanUnityAssets(inventory);
                await temp.ScanScriptFiles(inventory);
                await temp.ScanPrefabsAndData(inventory);

                inventory.Hash = temp.CalculateInventoryHash(inventory);
                await temp.SaveInventoryJson(inventory);

                return (OUTPUT_PATH, inventory.FacultySurfaces.Count);
            }
            finally
            {
        ...
        private async System.Threading.Tasks.Task ScanScriptFiles(InventoryData inventory)
        {
            ...
            {
                var relativePath = "Assets" + scriptPath[ Application.dataPath.Length.. ];
        ...
        private long GetFileSize(string path)
        {
            try
            {
                var fullPath = path.StartsWith("Assets") ?
                    Path.Combine(Application.dataPath, path[ 7.. ]) : path;
    ...
    {
        // Use public fields for Unity JsonUtility serialization
        public string Timestamp;
        public string Hash;
        public string UnityVersion;
        public string ProjectPath;
        public List<FacultySurface> FacultySurfaces = new List<FacultySurface>();
    }
