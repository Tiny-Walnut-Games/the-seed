    ...
    public class CompanionWarblerIntegration : MonoBehaviour
    {
        ...
        private void OnTurnStarted(Companion companion)
        {
            var context = new BattleContext
            {
                ...
                battleProgress = CalculateBattleProgress(),
            ...
            {
                // 🔧 LEGENDARY FIX: Store quip for companion to use - avoid direct event invocation
                StoreGeneratedQuip(companion, quip);
                Debug.Log($"[WarblerIntegration] Generated turn start quip for {companion.Name}: \"{quip}\"");
            }
        }
        ...
        private void OnBattleEnded(List<Companion> winners)
        {
            foreach (var winner in winners)
            {
                ...
                if (!string.IsNullOrEmpty(quip))
                {
                    // 🔧 LEGENDARY FIX: Store victory quip for companion
                    StoreGeneratedQuip(winner, quip);
                    Debug.Log($"[WarblerIntegration] Generated victory quip for {winner.Name}: \"{quip}\"");
                }
            }
        }
        ...
        private void OnAbilityUsed(Companion user, CompanionAbility ability)
        {
            var context = new BattleContext
            {
                ...
                usedAbility = ability,
                battleProgress = CalculateBattleProgress() // 🔧 Dynamic calculation
            };
            ...
            if (!string.IsNullOrEmpty(quip))
            {
                if (user != null)
                {
                    // 🔧 LEGENDARY FIX: Store ability quip for companion to use
                    StoreGeneratedQuip(user, quip);
                    
                    // 🎭 LEGENDARY ENHANCEMENT: Add battle log integration
                    Debug.Log($"[WarblerIntegration] {user.Name} uses {ability.name}: \"{quip}\"");
                }
                else
                {
                    Debug.LogWarning("[WarblerIntegration] Ability used by null companion");
                }
            }
        }
        ...
        /// <summary>
        /// 🔧 LEGENDARY WORKAROUND: Store generated quip for companion to use later
        /// Sacred Vision: Avoid direct event invocation by providing quips through other means
        /// </summary>
        private void StoreGeneratedQuip(Companion companion, string quip)
        {
            if (companionPersonalities.ContainsKey(companion))
            {
                // Store the quip in companion's personality context for potential use
                var personality = companionPersonalities[companion];
                // Could store in a "pending quips" queue or use other integration method
                
                // For now, just log - actual integration would depend on companion's quip system
                Debug.Log($"[WarblerIntegration] Stored quip for {companion.Name}: \"{quip}\"");
            }
        }
        ...
        /// <summary>
        /// 🔧 ENHANCEMENT: Calculate actual battle progress from current state
        /// Sacred Vision: Dynamic context for more accurate personality responses
        /// </summary>
        private float CalculateBattleProgress()
        {
            try
            {
                var allCompanions = FindObjectsOfType<Companion>();
                if (allCompanions.Length == 0) return 0.5f;
                
                // Use health percentage for accurate battle progress
                var totalStartingHealthPercent = allCompanions.Length; // Each companion starts at 100% = 1.0
                var currentHealthPercent = allCompanions.Where(c => c.IsAlive).Sum(c => c.BattleStats.healthPercentage);
                
                // Battle progress = 1 - (remaining health percentage / starting health percentage)
                return totalStartingHealthPercent > 0 ? 1f - (currentHealthPercent / totalStartingHealthPercent) : 0.5f;
            }
            catch (System.Exception e)
            {
                Debug.LogWarning($"[WarblerIntegration] Failed to calculate battle progress: {e.Message}");
                return 0.5f; // Safe fallback
            }
        }
        
        /// <summary>
        /// 🔧 Helper method to get current turn number
        /// </summary>
        private int GetCurrentTurn()
        {
            var battleManager = FindObjectOfType<CompanionBattleManager>();
            // Would need access to battle manager's turn counter
            // For now, return a placeholder value
            return UnityEngine.Random.Range(1, 10); // Temporary - would need public property access
        }
    }
}
