...
namespace LivingDevAgent.Runtime.CompanionBattler
{
    ...
    public class Companion : MonoBehaviour
    {
        ...
        [SerializeField] private List<CompanionAbility> abilities = new();
        ...
        [SerializeField] private List<string> battleStartQuips = new();
        [SerializeField] private List<string> victoryQuips = new();
        [SerializeField] private List<string> defeatQuips = new();
        [SerializeField] private List<string> lowHealthQuips = new();
        ...
        private void InitializeBattleStats(Dictionary<string, object> data)
        {
            ...
            {
                if (data [ "battle_stats" ] is Dictionary<string, object> battleData)
                {
        ...
        private int GetBaseStatForStage(string statType, string stage)
        {
            ...
            Dictionary<string, Dictionary<string, int>> stageStats = new()
            {
        ...
        public float GetElementalEffectiveness(CompanionElement targetElement)
        {
            ...
            {
                CompanionElement.Logic => targetElement == CompanionElement.Chaos ? 2.0f :
                                           targetElement == CompanionElement.Creativity ? 0.5f : 1.0f,
                CompanionElement.Creativity => targetElement == CompanionElement.Logic ? 2.0f :
                                           targetElement == CompanionElement.Order ? 0.5f : 1.0f,
                CompanionElement.Order => targetElement == CompanionElement.Creativity ? 2.0f :
                                           targetElement == CompanionElement.Chaos ? 0.5f : 1.0f,
                CompanionElement.Chaos => targetElement == CompanionElement.Order ? 2.0f :
                                           targetElement == CompanionElement.Logic ? 0.5f : 1.0f,
                _ => 1.0f,// Neutral effectiveness
            };
        }

        /// <summary>
        /// Public method to trigger battle quips from external systems like Warbler integration
        /// KeeperNote: Safe event invocation for external Warbler personality system
        /// </summary>
        public void SpeakQuip(string quip)
        {
            if (!string.IsNullOrEmpty(quip))
            {
                OnQuipSpoken?.Invoke(this, quip);
                Debug.Log($"[{companionName}]: \"{quip}\"");
            }
        }
        ...
        private string GenerateBattleDNA()
        {
            ...
            return Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(dnaInput))[ ..16 ];
        }
    }
}
