...
namespace MobileGameTemplate.UI
{
    ...
    public class LevelCarousel : VisualElement
    {
        ...
        private readonly List<LevelCarouselItem> _levelItems = new();
        ...
        // Configuration
        private readonly float _itemWidth = 200f;
        private readonly float _itemSpacing = 20f;
        private readonly float _snapThreshold = 0.3f;
        private readonly float _scrollDamping = 0.85f;
        private readonly float _animationSpeed = 8f;
        private readonly bool _enableHapticFeedback = true;
        #endregion
        ...
        void OnPointerMove(PointerMoveEvent evt)
        {
            ...
            Vector2 delta = new Vector2(evt.position.x, evt.position.y) - _lastPointerPosition;
            _velocity = delta;
        ...
        private void OnPointerUp(PointerUpEvent evt)
        {
            ...
            float targetPosition = _scrollPosition - momentum;
            
            // Find closest level and snap to it
            int targetLevel = FindClosestLevel();
            
            // Check if we should snap to target position based on velocity threshold
            float distanceToTarget = Mathf.Abs(CalculatePositionForLevel(targetLevel) - _scrollPosition);

            if (Mathf.Abs(momentum) > _snapThreshold * 100f && distanceToTarget < _snapThreshold)
            {
                targetPosition = CalculatePositionForLevel(targetLevel);
            }
            _targetScrollPosition = Mathf.Clamp(targetPosition, 0, Mathf.Max(0, _content.resolvedStyle.width - _viewport.resolvedStyle.width));
    ...
    /// <summary>
    /// Individual level item in the carousel
    /// </summary>
    public class LevelCarouselItem : VisualElement
    {
        ...
        private void CreateItemStructure()
        {
            ...
            //_background.style.borderTopLeftRadius = 16;
            //_background.style.borderTopRightRadius = 16;
            //_background.style.borderBottomLeftRadius = 16;
            //_background.style.borderBottomRightRadius = 16;
            ...
            _background.style.borderTopColor = new Color(0.3f, 0.3f, 0.3f, 0.8f);
            _background.style.borderRightColor = new Color(0.3f, 0.3f, 0.3f, 0.8f);
            _background.style.borderBottomColor = new Color(0.3f, 0.3f, 0.3f, 0.8f);
            _background.style.borderLeftColor = new Color(0.3f, 0.3f, 0.3f, 0.8f);
            ...
            // Note: borderRadius not available in Unity UIToolkit .NET Framework 4.7.1
            // _lockIcon.style.borderRadius = 20;
        ...
        private void UpdateVisualState()
        {
            ...
            if (_levelData.isLocked)
            {
                ...
                //_background.style.borderColor = new Color(0.5f, 0.5f, 0.5f, 0.8f);
            }
            else if (_levelData.isCompleted)
            {
                ...
                // Note: borderColor not available in Unity UIToolkit .NET Framework 4.7.1
                // _background.style.borderColor = new Color(0.3f, 0.7f, 0.8f, 0.8f);
            }
            else
            {
                _lockIcon.style.display = DisplayStyle.None;
                ...
                _playButton.SetEnabled(true);
                // Note: borderColor not available in Unity UIToolkit .NET Framework 4.7.1
                // _background.style.borderColor = new Color(0.3f, 0.3f, 0.7f, 0.8f);
            }
        }
        ...
        public void SetActive(bool active)
        {
            if (active)
            {
                ...
                // Note: borderColor not available in Unity UIToolkit .NET Framework 4.7.1
                // _background.style.borderColor = new Color(0.3f, 0.9f, 0.9f, 1f);
            }
            else
            {
