...
namespace MobileGameTemplate.Systems
{
    ...
    public class ShopSystem : VisualElement
    {
        ...
        private List<ShopItem> _allShopItems = new();
        private List<ShopItem> _currentItems = new();
        private ShopCategory _activeCategory = ShopCategory.General;
        private System.DateTime _nextRefreshTime;

        // ⚠ Intention ⚠ - @jmeyer1980 - These could be updated at runtime, so we leave them non-const
        // The IDE may tell you to make them readonly... ignore it.
        private int _itemsPerRow = 2;
        private float _itemWidth = 160f;
        private float _itemHeight = 200f;
        private readonly float _itemSpacing = 8f;
        #endregion
        ...
        private void SetupCurrencyDisplay()
        {
            ...
            _currencyContainer.style.flexWrap = Wrap.Wrap; // Allow wrapping on small screens
            _currencyContainer.style.minHeight = 40; // Ensure consistent height
            
            // Currency displays container
            ...
            currencyRow.style.flexWrap = Wrap.Wrap; // Allow currency items to wrap
            currencyRow.style.flexShrink = 0; // Prevent shrinking
            ...
            // Buy gems button container for better layout control
            ...
            _buyGemsButton.style.flexShrink = 0; // Prevent button shrinking
            _buyGemsButton.style.minWidth = 80; // Ensure minimum button width
            _buyGemsButton.style.whiteSpace = WhiteSpace.NoWrap; // Prevent text wrapping
            
            buttonContainer.Add(_buyGemsButton);
            
            _currencyContainer.Add(currencyRow);
            _currencyContainer.Add(buttonContainer);
        ...
        private VisualElement CreateCurrencyDisplay(string icon, string amount, Color iconColor)
        {
            ...
            container.style.backgroundColor = iconColor * 0.2f;
            container.style.paddingLeft = 12;
            container.style.paddingRight = 12;
            container.style.paddingTop = 8;
            container.style.paddingBottom = 8;
            container.style.marginRight = 8;
            container.style.minWidth = 80; // Prevent squishing
            container.style.flexShrink = 0; // Don't allow shrinking
            
            var iconLabel = new Label(icon);
            iconLabel.style.fontSize = 16;
            iconLabel.style.marginRight = 6;
            iconLabel.style.flexShrink = 0; // Keep icon size stable
            ...
            amountLabel.style.minWidth = 40; // Ensure minimum space for numbers
            amountLabel.style.flexShrink = 0; // Prevent text compression
            amountLabel.style.overflow = Overflow.Hidden; // Handle very long numbers gracefully
            amountLabel.style.unityTextAlign = TextAnchor.MiddleRight; // Right-align numbers
        ...
        private void SetupShopGrid()
        {
            ...
            // Note: borderRadius and borderWidth properties not available in Unity UIToolkit .NET Framework 4.7.1
            // _refreshButton.style.borderTopLeftRadius = 6;
            // _refreshButton.style.borderTopRightRadius = 6;
            // _refreshButton.style.borderBottomLeftRadius = 6;
            // _refreshButton.style.borderBottomRightRadius = 6;
            // _refreshButton.style.borderTopWidth = 0;
            // _refreshButton.style.borderRightWidth = 0;
            // _refreshButton.style.borderBottomWidth = 0;
            // _refreshButton.style.borderLeftWidth = 0;
        ...
        private VisualElement CreateShopItemCard(ShopItem item)
        {
            ...
            card.style.minHeight = _itemHeight; // Ensure consistent height
            card.style.maxHeight = _itemHeight; // Prevent vertical expansion
            card.style.overflow = Overflow.Hidden; // Clip content that exceeds bounds
            ...
            if (item.itemData?.itemIcon != null)
            {
                ...
                // Use available background properties for .NET Framework 4.7.1
                itemImage.style.backgroundSize = new BackgroundSize(BackgroundSizeType.Contain);
                itemImage.style.backgroundRepeat = new BackgroundRepeat(Repeat.NoRepeat, Repeat.NoRepeat);
            }
            
            // Item name with proper text handling
            ...
            itemName.style.height = 16; // Fixed height to prevent layout shift
            ...
            itemName.style.unityTextOverflowPosition = TextOverflowPosition.End; // Add ellipsis for long names
            
            // Quantity/Limited indicator with fixed positioning
            if (item.isLimited)
            {
                ...
                limitedLabel.style.height = 14; // Fixed height
                ...
                limitedLabel.style.overflow = Overflow.Hidden;
                card.Add(limitedLabel);
            }
            
            // Price buttons with better spacing
            ...
            priceContainer.style.height = 80; // Fixed height for price area
            priceContainer.style.justifyContent = Justify.FlexEnd; // Align buttons to bottom
        ...
        /// </summary>
        private Button CreatePriceButton(ShopItem item, CurrencyType currencyType, int price, string icon)
        {
            ...
            button.style.flexShrink = 0; // Prevent button compression
            button.style.minWidth = 60; // Ensure minimum button width
            button.style.overflow = Overflow.Hidden; // Handle long text gracefully
            ...
            // Button content container for better layout control
            ...
            buttonContent.style.flexShrink = 0;
        ...
        //#region Factory Methods // @copilot factory methods are obsolete, right? Remove?
        ...
        //    readonly UxmlFloatAttributeDescription _itemWidth = new()
        //    { 
        //        name = "item-width", 
        //        defaultValue = 160f 
        //    };

        //    readonly UxmlFloatAttributeDescription _itemHeight = new()
        //    { 
        //        name = "item-height", 
        //        defaultValue = 200f 
        //    };
